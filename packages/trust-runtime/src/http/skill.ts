...
export const SKILL_VERIFIED_DISTRIBUTION_RECORD_METHOD = "skill.distribution.record-verified" as const;
export const SKILL_RELEASE_AUTHORIZATION_SET_METHOD = "skill.release.authorization.set" as const;
export const SKILL_DEPLOYMENT_AUTHORIZATION_SET_METHOD = "skill.deployment.authorization.set" as const;
export const SKILL_DEPLOYMENT_ANNOUNCE_METHOD = "skill.deployment.announce" as const;
export const SKILL_DEPLOYMENT_SELECTION_SET_METHOD = "skill.deployment.selection.set" as const;
...
interface SkillReleaseClaimResult { readonly releaseDigest: string; readonly recordedAt: string }
interface VerifiedSkillDistributionRecordResult { readonly releaseDigest: string; readonly distributionDigest: string; readonly verifiedAt: string }
interface SkillAuthorizationSetResult { readonly decision: "ALLOW" | "REVOKE"; readonly effectiveAt: string }
interface SkillDeploymentSelectionSetResult { readonly environment: string; readonly requirement: SkillRequirement; readonly deploymentKey: string | null; readonly selectedAt: string | null }
interface SkillDeploymentAnnounceResult { readonly environment: string; readonly deploymentKey: string; readonly announcedAt: string; readonly recordedAt: string; readonly leaseExpiresAt: string }
interface SkillRequirementPreflightDto extends SkillRequirement {
  readonly status: string;
  readonly reasonCode: ReturnType<SkillPreflight["evaluateRequirement"]>["reasonCode"];
  readonly reason: string;
  readonly deploymentKey?: string;
  readonly skill?: string;
  readonly version?: string;
  readonly envelope?: "cli" | "mcp-stdio" | "mcp-http";
}
interface SkillEnvironmentPreflightResult {
  ...
  readonly summary: { readonly required: number; readonly ready: number; readonly blocked: number };
...
export interface RegistryAuthorityFailureData {
  ...
  readonly reason: import("../skill/authority.js").RegistryAuthorityErrorReason;
  readonly message: string;
}
...
export function executeSkillRegistryRpc(
  ...
  switch (method) {
    case SKILL_RELEASE_CLAIM_METHOD: {
      ...
      const registered = dependencies.skillRegistry.registerRelease(release);
      return {
    ...
    case SKILL_VERIFIED_DISTRIBUTION_RECORD_METHOD: {
      ...
      const verified = dependencies.skillRegistry.recordVerifiedDistribution(distribution);
      return {
    ...
    case SKILL_RELEASE_AUTHORIZATION_SET_METHOD: {
      ...
      const effectiveAt =
        input.decision === "ALLOW"
          ? dependencies.skillRegistry.authorizeRelease({
              environment: input.environment,
              releaseDigest: input.releaseDigest,
              authorizedBy: principal.identity,
            }).authorizedAt
          : (dependencies.skillRegistry.revokeRelease(
              input.environment,
              input.releaseDigest,
    ...
    case SKILL_DEPLOYMENT_AUTHORIZATION_SET_METHOD: {
      ...
      const effectiveAt =
        input.decision === "ALLOW"
          ? dependencies.skillRegistry.authorizeDeployment({
              environment: input.environment,
          ...
          : (dependencies.skillRegistry.revokeDeployment(
              input.environment,
              input.deploymentKey,
              input.releaseDigest,
              input.envelope,
    ...
    case SKILL_DEPLOYMENT_SELECTION_SET_METHOD: {
      ...
      if (input.deploymentKey === null) {
        dependencies.skillRegistry.clearSelection(input.environment, input.requirement);
        return {
          environment: input.environment,
      ...
      const selected = dependencies.skillRegistry.selectDeployment({
        environment: input.environment,
        ...input.requirement,
        logicalDeploymentKey: input.deploymentKey,
    ...
    case SKILL_DEPLOYMENT_ANNOUNCE_METHOD: {
      ...
      const recorded = dependencies.skillRegistry.announceDeployment(announcement);
      return {
    ...
    case SKILL_ENVIRONMENT_PREFLIGHT_METHOD:
      ...
      return projectEnvironmentPreflight(
        dependencies.skillPreflight.evaluateEnvironment(
...
function projectEnvironmentPreflight(
  result: ReturnType<SkillPreflight["evaluateEnvironment"]>,
...
function projectReasonCode(
  reason: ReturnType<SkillPreflight["evaluateRequirement"]>["reasonCode"],
