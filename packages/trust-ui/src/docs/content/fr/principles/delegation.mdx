...
summary: "Comment un agent exécute un Check sans produire sa propre preuve : lire le Plan, confier un Check au Runner, obtenir un résultat et sa continuation."
order: 1
---
...
sequenceDiagram
  ...
  T-->>R: qualification et prochaine action
  R-->>A: résultat et next
  opt next demande une resynchronisation
    A->>T: relit le Plan
  end
```
...
| --- | --- | --- |
| **Agent** | lit le Plan, choisit un Check actionnable, appelle le Runner avec, lit `result` et suit `next` | exécuter lui-même les commandes de l'Opération, fabriquer des Facts, inférer une progression depuis une sortie, décider qu'un Check est validé |
...
| --- | --- | --- |
| **completed · validated** | l'Opération s'est exécutée, les Facts ont été acceptés, le Check est satisfait | suivre `next` : exécuter le Check fourni, s'arrêter lorsque le Plan est complet, ou le relire lorsque c'est demandé |
| **completed · not validated** | l'Opération s'est exécutée, les Facts ont été acceptés, une garde de qualification est fausse — sa raison calculée explique l'observation | utiliser les champs `name`, `successReason` et `checkUri` fournis pour réessayer, ou escalader |
| **refused** | rien ne s'est exécuté : le Check n'est pas courant, ses prérequis sont ouverts, la Session est fermée, le contrat ou l'Environnement ne correspond pas | suivre la raison et `READ_PLAN` |
