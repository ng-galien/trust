...
A Check is qualified from a **complete batch**: every field its expectations name, and every field it materializes, must be present. An incomplete batch is **rejected as a whole** — no Fact, no record, no verdict, no change to the Plan. The execution may be repeated; that is the normal answer to a rejection.

Identical Facts submitted twice give the same record once: TRUST deduplicates the batch, the record and the verdict.

## Verdicts

The expectations of the Check are evaluated in table order against the Facts and the current Plan context.
...
- the Checks whose expectations compare with one of its fields.
