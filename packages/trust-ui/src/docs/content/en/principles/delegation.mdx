...
summary: "How an agent executes a Check without producing its own proof: read the Plan, hand one Check to the Runner, get a verdict, read again."
order: 1
---
...
| --- | --- | --- |
| **Agent** | reads the Plan, picks an actionable Check, calls the Runner with it, reads the result, reads the Plan again | run the Operation's commands itself, fabricate Facts, infer progress from an output, decide a Check is validated |
...
| --- | --- | --- |
| **completed · validated** | the Operation ran, the Facts were accepted, the Check is satisfied | read the Plan again: dependent Checks may have opened |
| **completed · not validated** | the Operation ran, the Facts were accepted, the expectations do not hold — the reason says which | address the observed condition; do not reinterpret it as progress |
| **refused** | nothing ran: the Check is not current, its prerequisites are open, the Session is closed, the contract or Environment does not match | read the Check or the Plan and follow the reason |
