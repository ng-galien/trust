...
export function ProcedureOverlay({ mode = "item" }: { mode?: "item" | "new" }) {
  ...
  const view = useOverlayViewState<Tab>(TABS, mode === "new" ? "source" : "overview");
  ...
  return (
    <ResourceOverlay
      ...
      id={seed ? t("procedures.overlay.duplicatedFrom", { id: displayId, from: seed.procedure.procedure }) : displayId}
      title={title}
      loading={
        procedures.isLoading ? <LoadingState /> : notFound ? (
          <div className="p-8">
            <EmptyState title={t("procedures.overlay.notFoundTitle", { id: id ?? "" })} body={t("procedures.overlay.notFoundBody")} action={<Button onClick={close}>{t("procedures.overlay.backToList")}</Button>} />
          </div>
        ) : undefined
      }
      actions={
        <>
          ...
          <Button
            ...
            title={!authoring ? t("procedures.overlay.publishHintEdit") : compileError ? t("procedures.overlay.publishHintFix") : t("procedures.overlay.publishHint")}
            onClick={onPublish}
          >
      ...
      tabs={[
        ...
        { value: "contract", label: <><Braces size={13} /> {t("procedures.overlay.tabs.contract")}</> },
      ]}
      tab={tab}
      onTab={setTab}
      tabMeta={
        ...
        : compiled ? `${plural(compiled.scenarios.length, "scenario")} · ${plural(compiled.checks.length, "check")} · ${plural(usedOperations.length, "operation")}` : ""
      }
      inspector={
        <>
          <InspectorSection title={t("procedures.overlay.executedBy")} count={executing.length}>
          ...
          <InspectorSection title={t("procedures.overlay.uses")} count={usedOperations.length}>
            ...
            {usedOperations.map((used) => (
              <RelationLink key={`${used.operation}@${used.version}`} to={`/operations/${encodeURIComponent(used.operation)}`} icon={<TerminalSquare />} title={used.operation} meta={t("procedures.overlay.usedMeta", { title: used.definition.title, version: used.version })} />
            ))}
          </InspectorSection>
          {compiled ? (
            <>
              ...
              <InspectorSection title={t("procedures.overlay.scenarios")} count={compiled.scenarios.length}>
    ...
    >
      {tab === "overview" ? <ProcedureOverview compiled={compiled} plans={executing} error={compileError?.detail} /> : null}
      {tab === "source" ? (
      ...
      {tab === "dag" ? (
        ...
        ) : <div className="p-6"><EmptyState icon={<Network />} title={t("procedures.overlay.noGraphTitle")} body={compileError?.detail ?? t("procedures.overlay.noGraphBody")} /></div>
      ) : null}
      {tab === "simulation" ? <ProcedureSimulation compiled={compiled} error={compileError?.detail} /> : null}
      {tab === "contract" ? (
        compiled ? (
          <div className="flex h-full min-h-0 flex-col">
            <div className="shrink-0 border-b border-border px-4 py-2 text-body text-muted"><span className="mono">{compiled.procedure}</span> {t("procedures.overlay.contractLine", { version: compiled.version, contract: compiled.contract, digest: compiled.definitionDigest.slice(0, 12) })}</div>
            <div className="min-h-0 flex-1"><JsonViewer value={compiled} /></div>
          </div>
        ) : <div className="p-6"><EmptyState icon={<Braces />} title={t("procedures.overlay.noContractTitle")} body={compileError?.detail ?? t("procedures.overlay.noContractBody")} /></div>
      ) : null}
