...
interface RevisionRow extends DatabaseRow {
  ...
  source: string;
  agent_declarations_json: string;
  agent_declaration_activations_json: string;
  role_incarnations_json: string;
  validated_outputs_json: string;
  validated_reports_json: string;
}
...
export interface PlanRepositoryDependencies {
...
export class PlanRepository {
  ...
  constructor({ databaseDriver }: PlanRepositoryDependencies) {
    this.#database = databaseDriver;
  }

  saveMaterializedRevision(compiled: MaterializedPlanRevision, compiledAt: string): void {
    this.#database.transaction(() => {
      ...
      for (const check of compiled.checks) {
        if (check.planSlug !== compiled.planSlug || check.planRevision !== compiled.revision) {
          throw new Error("a materialized Check must belong to its exact Plan revision");
        }
      }
      ...
      const revision = this.#database
        .prepare(
          `INSERT INTO plan_revisions (
             plan_slug, revision, definition_digest, source, agent_declarations_json,
             agent_declaration_activations_json, role_incarnations_json,
             validated_outputs_json, validated_reports_json, compiled_at
           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        )
        .run(
          compiled.planSlug,
          ...
          JSON.stringify(compiled.agentDeclarationActivations),
          JSON.stringify(compiled.roleIncarnations),
          JSON.stringify(compiled.validatedOutputs),
          JSON.stringify(compiled.validatedCheckObservations),
          compiledAt,
        );
      void revision;
  ...
  findCurrentCheck(checkUri: string): MaterializedCheck | undefined {
    const row = this.#database
      .prepare(
    ...
    return row ? (JSON.parse(row.check_json) as MaterializedCheck) : undefined;
  }

  listCurrentChecks(planSlug: string): MaterializedCheck[] {
    return this.#database
      ...
      .map((row) => JSON.parse(row.check_json) as MaterializedCheck);
  }

  findRevision(planSlug: string, revision: number): { id: number; definitionDigest: string } | undefined {
    const row = this.#database
      .prepare(
        `SELECT id, definition_digest
           FROM plan_revisions
          WHERE plan_slug = ? AND revision = ?`,
      )
      .get<RevisionRow>(planSlug, revision);
    return row ? { id: row.id, definitionDigest: row.definition_digest } : undefined;
  }
  ...
  findMaterializedRevision(
    planSlug: string,
    revision: number,
  ): MaterializedPlanRevision | undefined {
    const plan = this.findPlan(planSlug);
    if (!plan) return undefined;
    const row = this.#database
      .prepare(
        `SELECT id, definition_digest, source, agent_declarations_json,
                agent_declaration_activations_json, role_incarnations_json,
                validated_outputs_json, validated_reports_json
           FROM plan_revisions
          WHERE plan_slug = ? AND revision = ?`,
      )
      .get<RevisionRow>(planSlug, revision);
    ...
    const checks = this.#database
      ...
      .map((check) => JSON.parse(check.check_json) as MaterializedCheck);
    return {
      ...
      source: row.source,
      agentDeclarations: JSON.parse(row.agent_declarations_json) as MaterializedPlanRevision["agentDeclarations"],
      agentDeclarationActivations: JSON.parse(row.agent_declaration_activations_json) as MaterializedPlanRevision["agentDeclarationActivations"],
      roleIncarnations: JSON.parse(row.role_incarnations_json) as MaterializedPlanRevision["roleIncarnations"],
      validatedOutputs: JSON.parse(row.validated_outputs_json) as MaterializedPlanRevision["validatedOutputs"],
      validatedCheckObservations: JSON.parse(row.validated_reports_json) as MaterializedPlanRevision["validatedCheckObservations"],
      checks,
    };
  }
}
