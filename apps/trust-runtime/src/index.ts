...
import { normalizeAuthority } from "./domain/check-uri.js";
import { DEFAULT_SKILL_OPERABILITY_POLICY } from "./domain/skill-registry.js";
import { parseRegistryPrincipalConfigurations } from "./infrastructure/configured-registry-authority.js";
import type { SkillPolicy } from "./application/skill-admission-service.js";
import {
  parseExecutionEnvironments,
  readConfiguredExecutions,
} from "./infrastructure/configured-executions.js";
...
const configuredExecutions = process.env.TRUST_EXECUTIONS_DIRECTORY === undefined
  ? []
  : readConfiguredExecutions(process.env.TRUST_EXECUTIONS_DIRECTORY);
const executionEnvironments = parseExecutionEnvironments(
  process.env.TRUST_EXECUTION_ENVIRONMENTS_JSON,
);
