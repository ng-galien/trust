# language: en
@trust-dsl:1 @procedure:end-to-end-red-green @version:1.0.0
Feature: Validate a multi-project stack through an end-to-end Red-Green cycle
  ...
  Scenario: Establish the clean acceptance baseline before reading the ticket
    Then Check "acceptance baseline" uses Skill capability "git.head-read" on "acceptance project" as input "repository" and materializes "acceptance baseline commit" from output "head revision" and must establish "the clean acceptance baseline is established before ticket investigation"
  ...
  @scenario:jira-issue
  Scenario: Read the Jira defect after establishing the acceptance baseline
  ...
  Scenario: Commit every declared ticket Scenario after the acceptance baseline
    Given scenario "jira-issue" is validated
    And scenario "acceptance-baseline" is validated
    Then Check "acceptance test comparison" uses Skill capability "git.head-compare" on "acceptance project" as input "repository" using "acceptance baseline commit" as input "base revision" and materializes "acceptance test commit" from output "head revision" and must establish "the declared ticket Scenarios are committed after the clean acceptance baseline"
      | observation            | relation | expectation                          | failure feedback                                   |
