# language: en
@trust-dsl:1 @operation:git.change-merge @version:1.1.0
Feature: Merge a ticket branch into main and delete the local branch
  ...
  deletes the merged local branch with `git branch --delete <branch>`; finally it reads HEAD with
  `git rev-parse --verify HEAD` and the working tree with `git status --porcelain=v1`. Merge exit 0
  is the observation `merged`; exit 1 is `failed`: a
  ...
  `mergedRevision` is the HEAD after the merge, so a Procedure can record what `main` became.
  Branch deletion exit 0 is `deleted`; exit 1 is `kept`, which lets the Procedure refuse completion
  until local cleanup succeeds. A branch with no commit ahead of `main` merges as `Already up to
  date` (exit 0, `merged`) and can still be deleted. Nothing is fetched or pushed: the local
  repository is the only remote for now.
  ...
  Background: Operation interface
    ...
    And Produced fields
      ...
      | branchStatus   | string    | one         | enum "deleted", "kept"  |
  ...
  Scenario: Run
    ...
    And Shell "merge" accepts exits
      ...
      | 0         |                 |                 |
      | 1         |                 |                 |
    And Shell "delete" runs "git" with cwd from Environment "workspaceRoot" and Input "project"
      | argument | source         |
      | branch   | literal        |
      | --delete | literal        |
      | branch   | Input "branch" |
    And Shell "delete" accepts exits
      | exit code | stdout contains | stderr contains |
      | 0         |                 |                 |
      | 1         |                 |                 |
    ...
    Then Produce with JSONata
      """
      {
        ...
        "branchStatus": steps.delete.exitCode = 0 ? "deleted" : "kept",
